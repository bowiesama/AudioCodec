...
typedef struct{
    int energy_dev_tol;
    int resolution_supgain;
    int supgain_default;
    int supgain_err_para_a;
    int supgain_err_para_b;
    int supgain_err_para_c;
    int supgain_err_para_d;
    int supgain_epc_dt;
} AecmCore_NLP_Param_t;
...
typedef struct {
    ...
    float H[PART_LEN2];
    float SmoothFactor;
    float full_band_noise_far;
    int  aecm_fix_run_times;
}AecmFloat;
#endif
typedef struct {
	float real;
...
typedef struct {
	...
	float far_history[PART_LEN1 * MAX_DELAY];
	int far_history_pos;
	...
	float nearLogEnergy[MAX_BUF_LEN];
	float farLogEnergy;
	float echoAdaptLogEnergy[MAX_BUF_LEN];
	float echoStoredLogEnergy[MAX_BUF_LEN];

	float channelStored[PART_LEN1];
	float channelAdapt16[PART_LEN1];
	float channelAdapt32[PART_LEN1];
	float echoFilt[PART_LEN1];
	float nearFilt[PART_LEN1];
	...
	float outBuf_buf[PART_LEN + 8];

	short startupState;
	short mseChannelCount;
	float supGain;
	float supGainOld;

	float supGainErrParamA;
	float supGainErrParamD;
	float supGainErrParamDiffAB;
	float supGainErrParamDiffBD;
} AecmCore_float;
...
void UpdateFarHistoryFloat(AecmCore_float* self,float* far_spectrum);
#endif

int WebRtcAecm_CreateCore(AecmCore_t **aecm, AecmCore_NLP_Param_t * param);
