...
static AecmCore_NLP_Param_t s_AecmCore_NLP_Param={
    400,
    8,
    256,     //1 in Q.8
    3072,    //12 in Q.8
...
static const ALIGN8_BEG WebRtc_Word16 WebRtcAecm_kSqrtHanning128[] ALIGN8_END =
{
...
int WebRtcAecm_CreateCore(AecmCore_t **aecmInst, AecmCore_NLP_Param_t * param)
{
    if(param != NULL){
        s_AecmCore_NLP_Param = *param;
    }
...
#ifdef	ALY_DBGX
    fnlp = fopen("pnlpx.raw", "wb");
    fhnl = fopen("phnlx.raw", "wb");
    ffe = fopen("fex.raw", "wb");
    fw = fopen("wnx.raw", "wb");
    fgn = fopen("gainx.raw", "wb");
    fvad = fopen("vadx.raw", "wb");
    ffv = fopen("fvx.raw", "wb");
    fnclean = fopen("ncx.raw", "wb");
    ffnear = fopen("fnearx.raw", "wb");
    ffest = fopen("festx.raw", "wb");
    ffeng = fopen("ffengx.raw", "wb");
    fdb = fopen("fdbx.raw", "wb");
    fy = fopen("fy.raw", "wb");
    fNumNlp = fopen("fNumNlp.raw", "wb");
#endif
    return 0;
}
...
int WebRtcAecm_InitCore(AecmCore_t * const aecm, int samplingFreq)
{
    ...
    aecm->startupState = 0;
    aecm->supGain = s_AecmCore_NLP_Param.supgain_default;
    aecm->supGainOld = s_AecmCore_NLP_Param.supgain_default;

    aecm->supGainErrParamA = s_AecmCore_NLP_Param.supgain_err_para_a;
    aecm->supGainErrParamD = s_AecmCore_NLP_Param.supgain_err_para_d;
    aecm->supGainErrParamDiffAB = s_AecmCore_NLP_Param.supgain_err_para_a - 
                                  s_AecmCore_NLP_Param.supgain_err_para_b;
    aecm->supGainErrParamDiffBD = s_AecmCore_NLP_Param.supgain_err_para_b - 
                                  s_AecmCore_NLP_Param.supgain_err_para_d;
    ...
    aecm->aecmfloat.SmoothFactor=0.5f;
#endif
...
#ifdef	ALY_DBGX
  	fwrite(&aecm->farEnergyVAD, 1, sizeof(short), ffv);
  	fwrite(&aecm->farLogEnergy, 1, sizeof(short), ffe);
#endif
...
static WebRtc_Word16 CalcSuppressionGain(AecmCore_t * const aecm)
{
...
#ifdef	ALY_DBGX
	short     vad = (short)(32700*aecm->currentVADValue);
  	fwrite(&vad, 1, sizeof(short), fvad);
#endif
    if (!aecm->currentVADValue)
    {
        supGain = 0;
    } else
    {
        ...
        if (dE < s_AecmCore_NLP_Param.energy_dev_tol)
        {
            // Likely no double talk. The better estimation, the more we can suppress signal.
            // Update counters
            if (dE < s_AecmCore_NLP_Param.supgain_epc_dt)
            {
                tmp32no1 = WEBRTC_SPL_MUL_16_16(aecm->supGainErrParamDiffAB, dE);
                tmp32no1 += (s_AecmCore_NLP_Param.supgain_epc_dt >> 1);
                tmp16no1 = (WebRtc_Word16)WebRtcSpl_DivW32W16(tmp32no1, s_AecmCore_NLP_Param.supgain_epc_dt);
                supGain = aecm->supGainErrParamA - tmp16no1;
            } else
            {
                tmp32no1 = WEBRTC_SPL_MUL_16_16(aecm->supGainErrParamDiffBD,
                                                (s_AecmCore_NLP_Param.energy_dev_tol - dE));
                tmp32no1 += ((s_AecmCore_NLP_Param.energy_dev_tol - s_AecmCore_NLP_Param.supgain_epc_dt) >> 1);
                tmp16no1 = (WebRtc_Word16)WebRtcSpl_DivW32W16(tmp32no1, (s_AecmCore_NLP_Param.energy_dev_tol
                        - s_AecmCore_NLP_Param.supgain_epc_dt));
    ...
    {
        aecm->supGain += (WebRtc_Word16)((tmp16no1 - aecm->supGain) >> 5);
    } else
    {
                            ...
                            WebRtc_Word16 * output)
{
...
#ifdef	ALY_DBGX
	if (fgn != NULL)
		fwrite(&supGain, 1, sizeof(short), fgn);
#endif
    ...
    if (aecm->nlpFlag)
    {
        float bef = 0, aft = 0;
#ifdef	ALY_DBGX
        float dataf[PART_LEN];
        short datas[PART_LEN];

        for (i = 0; i < PART_LEN; ++i)
            datas[i] = numPosCoef;
        fwrite(&datas[0], 1, PART_LEN*sizeof(short), fNumNlp);
#endif        
        for (i = 0; i < PART_LEN1; i++)
        {
            ...
            efw[i].imag = (WebRtc_Word16)(WEBRTC_SPL_MUL_16_16_RSFT_WITH_ROUND(dfw[i].imag,
                                                                            hnl[i], 14));
            bef += (float)(dfw[i].real * dfw[i].real);
            bef += (float)(dfw[i].imag * dfw[i].imag);
            aft += (float)(efw[i].real * efw[i].real);
            aft += (float)(efw[i].imag * efw[i].imag);
#ifdef	ALY_DBGX
            if (fnlp != NULL)
                fwrite(&nlpGain, 1, sizeof(short), fnlp);
            if (fhnl != NULL)
                fwrite(&hnl[i], 1, sizeof(short), fhnl);
#endif
        }
        
        if ((aft/bef) < (1040.0/32768.0))
        {
            for (i = 0; i < PART_LEN1; i++)
            {
                efw[i].real = 0;
        ...
        for (i = 0; i < PART_LEN; ++i)
            dataf[i] = (aft/bef);
        fwrite(&dataf[0], 1, PART_LEN*sizeof(float), fy);
#endif
...
#ifdef FLOATPRO
void CopyFixFFT2Float(complex16_t* freq_signal,int16_t zeros,float* fp){
    int i;
...
void NoiseTrack(AecmCore_t* aecm)
{
    int i;
    //float temp[PART_LEN] ={0};
    ...
    for(i=0;i<PART_LEN;i++) {
        ...
        sumenoise += aecm->aecmfloat.FarNoise[i];
        sumpower +=  aecm->aecmfloat.FarPower[i];
    }
    aecm->aecmfloat.full_band_noise_far += 0.3*(sumenoise - aecm->aecmfloat.full_band_noise_far);
    if (sumpower>4.f* aecm->aecmfloat.full_band_noise_far ) {
        aecm->aecmfloat.aecm_fix_run_times = ++aecm->aecmfloat.aecm_fix_run_times>AECM_FIX_MAX_RUN? AECM_FIX_MAX_RUN:aecm->aecmfloat.aecm_fix_run_times;
    }
    
    
}
void UpdateH(AecmCore_t* aecm) {
    int i,delay;
    float real,imag;
...
void CopyFloatFFT2Fix(complex16_t* freq_signal,int16_t zeros,float* fp) {
    int i,tempvalue;
...
int WebRtcAecm_ProcessBlockwithFloat(AecmCore_t* aecm,
...
#ifdef FLOATPRO
    if(1==aecm->enablefloatpro && nearendClean == NULL) {
...
#ifdef FLOATPRO
        if(1==aecm->enablefloatpro) {
            CopyFixFFT2Float(dfw,aecm->dfaCleanQDomain,aecm->aecmfloat.NearSpectrum);
        }
#endif
    }
...
#ifdef FLOATPRO
    if (1 == aecm->enablefloatpro && (aecm->currentDelay>1 || aecm->aecmfloat.aecm_fix_run_times >= AECM_FIX_MAX_RUN )) {
        //delay =(aecm->aecmfloat.FarWritePosition - aecm->currentDelay + MAX_DELAY)%MAX_DELAY * PART_LEN2;
