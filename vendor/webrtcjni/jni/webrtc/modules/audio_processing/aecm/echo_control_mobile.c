...
WebRtc_Word32 WebRtcAecm_Create(void **aecmInst, void * param)
{
    aecmob_t *aecm;
    if (aecmInst == NULL)
    {
    ...
    *aecmInst = aecm;
    ...
    if (WebRtcAecm_CreateCore(&aecm->aecmCore,param) == -1)
    {
...
#ifdef USE_FLOATING_POINT
		if (WebRtcAecm_CreateCoreFloat(&aecm->aecmCoreFloat) == -1)
		{
			WebRtcAecm_Free(aecm);
			aecm = NULL;
			return -1;
...
#ifdef USE_FLOATING_POINT
	if (WebRtcAecm_InitCoreFloat(aecm->aecmCoreFloat, aecm->sampFreq) == -1)
	{
		aecm->lastError = AECM_UNSPECIFIED_ERROR;
		return -1;
	}
                                 ...
                                 WebRtc_Word16 nrOfSamples, WebRtc_Word16 msInSndCardBuf)
{
    ...
    } else
    {
        ...
        for (i = 0; i < nFrames; i++)
        {
            ...
            {
#ifdef USE_FLOATING_POINT
				if(WebRtcAecm_ProcessFrameFloat(aecm->aecmCoreFloat,	farend_ptr,nearendNoisy[FRAME_LEN * i],	NULL,	&out[FRAME_LEN * i])==-1)
					return -1;
#else
...
#ifdef USE_FLOATING_POINT
				if ((retVal=WebRtcAecm_ProcessFrameFloat(aecm->aecmCoreFloat,
                                    farend_ptr,
                                    &nearendNoisy[FRAME_LEN * i],
					...
					return -1;
#else			
				if ((retVal=(WebRtcAecm_ProcessFrame(aecm->aecmCore,
                                            farend_ptr,
                                            &nearendNoisy[FRAME_LEN * i],
                                            &nearendClean[FRAME_LEN * i],
                                            &out[FRAME_LEN * i]))) == -1)
...
#ifdef USE_FLOATING_POINT
	aecm->aecmCoreFloat->cngMode = config.cngMode;
#endif
...
WebRtc_Word16 WebRtcAecm_get_echo_suppression_gain(void *aecmInst)
{
    aecmob_t *aecm = aecmInst;
    return aecm->aecmCore->supGainOld;
}
