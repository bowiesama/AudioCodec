// aecmad.cpp : Defines the entry point for the console application.
//
  ...
  if (pb_file) 
  {
    ...
    while (ReadMessageFromFile(pb_file, &event_msg)) 
	{
      ...
      if (event_msg.type() == Event::INIT) {
		...
		msg.has_device_sample_rate();
        //ASSERT_EQ(apm->kNoError,
        //          apm->echo_cancellation()->set_device_sample_rate_hz(
        //              msg.device_sample_rate()));
		apm->echo_cancellation()->set_device_sample_rate_hz(msg.device_sample_rate());

        //ASSERT_TRUE(msg.has_num_input_channels());
		msg.has_num_input_channels();
        //ASSERT_TRUE(msg.has_num_output_channels());
		msg.has_num_output_channels();
        //ASSERT_EQ(apm->kNoError,
        //    apm->set_num_channels(msg.num_input_channels(),
        //                          msg.num_output_channels()));
		apm->set_num_channels(msg.num_input_channels(),msg.num_output_channels());
       ...
       // ASSERT_EQ(apm->kNoError,
        ...
        if (vad_out_file != NULL) {
          ...
          //                     1,
          //                     vad_out_file));
		  fwrite(&stream_has_voice,sizeof(stream_has_voice),1,vad_out_file);
        }
        ...
        if (ns_prob_file != NULL) {
          ...
          //                     1,
          //                     ns_prob_file));
		  fwrite(&ns_speech_prob,sizeof(ns_speech_prob),1,ns_prob_file);
        }
        ...
        //                       size,
        //                       out_file));
		fwrite(near_frame.data_,sizeof(int16_t),size,out_file);
      }
    }
  ...
  else 
  {
    ...
    while (simulating || feof(event_file) == 0) {
      ...
      if (simulating) {
        ...
        read_count = fread(&event, sizeof(event)/2, 1, event_file);			// modefied  by chen
        if (read_count != 1) {
          break;
        }
      }
      ...
      if (event == kInitializeEvent || event == kResetEventDeprecated) {
        ...
        //          1,
        //          event_file));
		fread(&device_sample_rate_hz,sizeof(device_sample_rate_hz),1,event_file);
