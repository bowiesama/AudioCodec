...
void WebRtcAec_ProcessFrame(AecCore* aec,
    ...
    // 4) Process as many blocks as possible.分块处理，滤波器长度？12块，每块64点
...
static void ProcessBlock(AecCore* aec) {
    ...
    // ---------- Ooura fft ----------
    // Concatenate old and new nearend blocks.读入近端数据
    for (i = 0; i < PART_LEN; i++) {
    ...
    // We should always have at least one element stored in |far_buf|.读入远端数据
    assert(WebRtc_available_read(aec->far_buf) > 0);
    WebRtc_ReadBuffer(aec->far_buf, (void**) &xf_ptr, &xf[0][0], 1);

    // Near fft 对近端做128点FFT（与上次的64点数据结合在一起）
    ...
    // Estimate noise power. Wait until dPow is more stable.估计噪声功率进行平滑以避免突变
    ...
    // Block wise delay estimation used for logging估计delay
