...
void ApmTest::SetUp() {
  ...
  Init(32000, 2, 2, 2, true);///true=false
}
...
TEST_F(ApmTest, EchoCancellationReportsCorrectDelays) {
  // Enable AEC only.
  EXPECT_EQ(apm_->kNoError,
            apm_->echo_cancellation()->enable_drift_compensation(false)); ///true
  EXPECT_EQ(apm_->kNoError,
            apm_->echo_cancellation()->enable_metrics(false));///true
  EXPECT_EQ(apm_->kNoError,
            apm_->echo_cancellation()->enable_delay_logging(true));///true
  EXPECT_EQ(apm_->kNoError, apm_->echo_cancellation()->Enable(true));///true
