...
)
{
  ...
  Word16 *scal_sig;
  Flag   Overflow = 0; // DK original Overflow usage
   ...
   *--------------------------------------------------------*/
   ...
   for(i= -pit_max; i< L_frame; i+=2)
     sum = L_mac(sum, signal[i], signal[i], &Overflow);
   ...
   *--------------------------------------------------------*/
   ...
   if(Overflow == 1)
   {
     ...
     {
       scal_sig[i] = shr(signal[i], 3, &Overflow);
     }
   }
   else {
     L_temp = L_sub(sum, (Word32)1048576L, &Overflow);
     if ( L_temp < (Word32)0 )  /* if (sum < 2^20) */
     {
        for(i=-pit_max; i<L_frame; i++)
        {
          scal_sig[i] = shl(signal[i], 3, &Overflow);
        }
     }
     else
     {
    ...
    *--------------------------------------------------------------------*/
    ...
    for (i = 20; i < 40; i++) {
        ...
        for (j=0; j<L_frame; j+=2, p+=2, p1+=2)
            sum = L_mac(sum, *p, *p1, &Overflow);
        L_temp = L_sub(sum, max, &Overflow);
    ...
    *-----------------------*/
    ...
    i = sub(shl(T2,1, &Overflow), T3, &Overflow);
    j = sub(abs_s(i), 5, &Overflow);
    if(j < 0)
      max2 = add(max2, shr(max3, 2, &Overflow), &Overflow);

    /* if( abs(T2*3 - T3) < 7)  */
    /*    max2 += max3 * 0.25;  */

    i = add(i, T2, &Overflow);
    j = sub(abs_s(i), 7, &Overflow);
    if(j < 0)
      max2 = add(max2, shr(max3, 2, &Overflow), &Overflow);
    ...
    i = sub(shl(T1,1, &Overflow), T2, &Overflow);
    j = sub(abs_s(i), 5, &Overflow);
    if(j < 0)
      max1 = add(max1, mult(max2, 6554, &Overflow), &Overflow);
    ...
    *--------------------------------------------------------------------*/
    ...
    if( sub(max1, max2, &Overflow) < 0 ) {max1 = max2; T1 = T2;  }
    if( sub(max1, max3, &Overflow) <0 )  {T1 = T3; }
...
)
{
  ...
  sum = 0;
  for(i=0; i<lg; i++)
    sum = L_mac(sum, x[i], y[i], &Overflow);
...
)
{
  ...
  *-----------------------------------------------------------------*/
  ...
  for(t=t0_min; t<=t0_max; t++)
  {
    ...
    L_temp = L_sub(corr, max, &Overflow);
  ...
  *pit_frac = 0;
  ...
  if( (i_subfr == 0) && (sub(t0, 84, &Overflow) > 0) )
    return t0;
...
)
{
   ...
   for(i=0; i<L_subfr; i++)
     scaled_y1[i] = shr(y1[i], 2, &Overflow);
   ...
   for(i=0; i<L_subfr; i++)
     s = L_mac(s, y1[i], y1[i], &Overflow);

   if (Overflow == 0) {
     exp_yy = norm_l(s);
     yy     = W_round( L_shl(s, exp_yy, &Overflow), &Overflow);
   }
   else {
     s = 1;                  /* Avoid case of all zeros */
     for(i=0; i<L_subfr; i++)
       s = L_mac(s, scaled_y1[i], scaled_y1[i], &Overflow);
     exp_yy = norm_l(s);
     yy     = W_round( L_shl(s, exp_yy, &Overflow), &Overflow);
     exp_yy = sub(exp_yy, 4, &Overflow);
   }
   ...
   s = 0;
   for(i=0; i<L_subfr; i++)
     s = L_mac(s, xn[i], y1[i], &Overflow);

   if (Overflow == 0) {
     exp_xy = norm_l(s);
     xy     = W_round( L_shl(s, exp_xy, &Overflow), &Overflow);
   }
   else {
     s = 0;
     for(i=0; i<L_subfr; i++)
       s = L_mac(s, xn[i], scaled_y1[i], &Overflow);
     exp_xy = norm_l(s);
     xy     = W_round( L_shl(s, exp_xy, &Overflow), &Overflow);
     exp_xy = sub(exp_xy, 2, &Overflow);
   }

   g_coeff[0] = yy;
   g_coeff[1] = sub(15, exp_yy, &Overflow);
   g_coeff[2] = xy;
   g_coeff[3] = sub(15, exp_xy, &Overflow);
   ...
   xy = shr(xy, 1, &Overflow);             /* Be sure xy < yy */
   gain = div_s( xy, yy);

   i = sub(exp_xy, exp_yy, &Overflow);
   gain = shr(gain, i, &Overflow);         /* saturation if > 1.99 in Q14 */

   /* if(gain >1.2) gain = 1.2  in Q14 */

   if( sub(gain, 19661, &Overflow) > 0)
   {
     gain = 19661;
   }
...
)
{
  ...
  if (pit_flag == 0)   /* if 1st subframe */
  {
    ...
    if (sub(T0, 85, &Overflow) <= 0)
    {
      /* index = t0*3 - 58 + t0_frac   */
      i = add(add(T0, T0, &Overflow), T0, &Overflow);
      index = add(sub(i, 58, &Overflow), T0_frac, &Overflow);
    }
    else {
      index = add(T0, 112, &Overflow);
    }

    /* find T0_min and T0_max for second subframe */

    *T0_min = sub(T0, 5, &Overflow);
    if (sub(*T0_min, pit_min, &Overflow) < 0)
    {
      *T0_min = pit_min;
    }

    *T0_max = add(*T0_min, 9, &Overflow);
    if (sub(*T0_max, pit_max, &Overflow) > 0)
    {
      *T0_max = pit_max;
      *T0_min = sub(*T0_max, 9, &Overflow);
    }
  }
  else      /* if second subframe */
  {
    ...
    /* i = t0 - t0_min;               */
    /* index = i*3 + 2 + t0_frac;     */
    i = sub(T0, *T0_min, &Overflow);
    i = add(add(i, i, &Overflow), i, &Overflow);
    index = add(add(i, 2, &Overflow), T0_frac, &Overflow);
  }
