...
)
{
  ...
  L_x = L_mult(fraction, 32, &Overflow);           /* L_x = fraction<<6           */
  i   = extract_h(L_x);                 /* Extract b10-b15 of fraction */
  L_x = L_shr(L_x, 1, &Overflow);
  a   = extract_l(L_x);                 /* Extract b0-b9   of fraction */
  a   = a & (Word16)0x7fff;

  L_x = L_deposit_h(tabpow[i]);         /* tabpow[i] << 16        */
  tmp = sub(tabpow[i], tabpow[i+1], &Overflow);    /* tabpow[i] - tabpow[i+1] */
  L_x = L_msu(L_x, tmp, a, &Overflow);             /* L_x -= tmp*a*2        */

  exp = sub(30, exponent, &Overflow);
  L_x = L_shr_r(L_x, exp, &Overflow);
...
)
{
  ...
  exp = norm_l(L_x);
  L_x = L_shl(L_x, exp , &Overflow);               /* L_x is normalized */

  *exponent = sub(30, exp, &Overflow);

  L_x = L_shr(L_x, 9, &Overflow);
  ...
  tmp = sub(tablog[i], tablog[i+1], &Overflow);    /* tablog[i] - tablog[i+1] */
  L_y = L_msu(L_y, tmp, a, &Overflow);             /* L_y -= tmp*a*2        */
...
)
{
  ...
  exp = norm_l(L_x);
  L_x = L_shl(L_x, exp, &Overflow );               /* L_x is normalize */

  exp = sub(30, exp, &Overflow);
  if( (exp & 1) == 0 )                  /* If exponent even -> shift right */
      L_x = L_shr(L_x, 1, &Overflow);

  exp = shr(exp, 1, &Overflow);
  exp = add(exp, 1, &Overflow);
  ...
  tmp = sub(tabsqr[i], tabsqr[i+1], &Overflow);    /* tabsqr[i] - tabsqr[i+1])  */
  L_y = L_msu(L_y, tmp, a, &Overflow);             /* L_y -=  tmp*a*2         */

  L_y = L_shr(L_y, exp, &Overflow);                /* denormalization */
