                ...
                )
{
  ...
  if (lsf[0] < L_LIMIT)
    lsf[0] = L_LIMIT;
  for (i=0 ; i < M-1 ; i++)
    if (sub(lsf[i+1], lsf[i], &Overflow) < 2*GAP3) 
      lsf[i+1] = add(lsf[i], 2*GAP3, &Overflow);
  if (lsf[M-1] > M_LIMIT)
    lsf[M-1] = M_LIMIT;
  if (lsf[M-1] < lsf[M-2]) 
    lsf[M-2] = sub(lsf[M-1], GAP3, &Overflow);
...
)
{
  ...
  *Pptr = ptr_back[0][ptr];
  ...
  for (i=0; i<M/2; i++)
    qlsf[i] = add(qlsf[i], lspcb2[PtrTab_2[0][cluster[1]]][i], &Overflow);
  for (i=M/2; i<M; i++)
    qlsf[i] = add(qlsf[i], lspcb2[PtrTab_2[1][cluster[1]]][i], &Overflow);
...
)
{
  ...
  for (p=0; p<J; p++)
    for (m=0; m<MQ; m++){
      acc0 = 0;
      for (l=0; l<M; l++){
        tmp = sub(d_data[p*M+l], lspcb1[PtrTab[m]][l], &Overflow);
        acc0 = L_mac(acc0, tmp, tmp, &Overflow);
      }
      sum[p*MQ+m] = extract_h(acc0);
      sum[p*MQ+m] = mult(sum[p*MQ+m], Mp[p], &Overflow);
    }
  ...
  for (q=0; q<K; q++){
    ...
    for (p=0; p<J; p++)
      for (m=0; m<MQ; m++)
        if (sub(sum[p*MQ+m], min[q], &Overflow) < 0){
          min[q] = sum[p*MQ+m];
		  min_indx_p[q] = p;
          min_indx_m[q] = m;          
        }
  ...
  for (q=0; q<K; q++){
    for (l=0; l<M; l++)
      new_d_data[q*M+l] = sub(d_data[min_indx_p[q]*M+l], 
                              lspcb1[PtrTab[min_indx_m[q]]][l], &Overflow);
...
)
{
    ...
    for (m=0; m<MQ; m++){
      ...
      for (l=0; l<M/2; l++){
        tmp1 = extract_h(L_shl(L_mult(noise_fg_sum[ptr_prd[p]][l], 
                                      noise_fg_sum[ptr_prd[p]][l], &Overflow), 2, &Overflow));
        tmp1 = mult(tmp1, weight[l], &Overflow);
        tmp2 = sub(d_data[p*M+l], lspcb2[PtrTab[0][m]][l], &Overflow);
        tmp1 = extract_h(L_shl(L_mult(tmp1, tmp2, &Overflow), 3, &Overflow));
        acc0 = L_mac(acc0, tmp1, tmp2, &Overflow);
      }
      ...
      for (l=M/2; l<M; l++){
        tmp1 = extract_h(L_shl(L_mult(noise_fg_sum[ptr_prd[p]][l], 
                                      noise_fg_sum[ptr_prd[p]][l], &Overflow), 2, &Overflow));
        tmp1 = mult(tmp1, weight[l], &Overflow);
        tmp2 = sub(d_data[p*M+l], lspcb2[PtrTab[1][m]][l], &Overflow);
        tmp1 = extract_h(L_shl(L_mult(tmp1, tmp2, &Overflow), 3, &Overflow));
        acc0 = L_mac(acc0, tmp1, tmp2, &Overflow);
      }
  ...
  for (q=0; q<K; q++){
    for (l=0; l<M/2; l++)
      new_d_data[q*M+l] = sub(d_data[min_indx_p[q]*M+l], 
                              lspcb2[PtrTab[0][min_indx_m[q]]][l], &Overflow);
    for (l=M/2; l<M; l++)
      new_d_data[q*M+l] = sub(d_data[min_indx_p[q]*M+l], 
                              lspcb2[PtrTab[1][min_indx_m[q]]][l], &Overflow);
