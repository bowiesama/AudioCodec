...
)
{
  ...
  if( erase==0 ) {  /* Not frame erasure */
    mode_index = shr(prm[0] ,NC0_B, &Overflow) & (Word16)1;
    code0 = prm[0] & (Word16)(NC0 - 1);
    code1 = shr(prm[1] ,NC1_B, &Overflow) & (Word16)(NC1 - 1);
...
void Get_decfreq_prev(DecState* decoder, Word16 x[MA_NP][M])
{
  ...
  for (i=0; i<MA_NP; i++)
    Copy(&decoder->freq_prev[i][0], &x[i][0], M);
}
...
void Update_decfreq_prev(DecState* decoder, Word16 x[MA_NP][M])
{
  ...
  for (i=0; i<MA_NP; i++)
    Copy(&x[i][0], &decoder->freq_prev[i][0], M);
}
