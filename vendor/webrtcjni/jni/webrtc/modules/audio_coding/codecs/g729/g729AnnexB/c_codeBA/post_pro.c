...
{
  Word16 *signal = decoder->synth; /* input/output signal */
  Word16 lg = L_FRAME;             /* length of signal    */
  Word16 i, x2;
  Word32 L_tmp;
  Flag   Overflow = 0;
  ...
  for(i=0; i<lg; i++)
  {
     ...
     L_tmp     = L_add(L_tmp, Mpy_32_16(decoder->y2_hi, decoder->y2_lo, a100[2]), &Overflow);
     L_tmp     = L_mac(L_tmp, decoder->x0, b100[0], &Overflow);
     L_tmp     = L_mac(L_tmp, decoder->x1, b100[1], &Overflow);
     L_tmp     = L_mac(L_tmp, x2, b100[2], &Overflow);
     L_tmp     = L_shl(L_tmp, 2, &Overflow);      /* Q29 --> Q31 (Q13 --> Q15) */

     /* Multiplication by two of output speech with saturation. */
     signal[i] = W_round(L_shl(L_tmp, 1, &Overflow), &Overflow);
