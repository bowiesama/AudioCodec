...
)
{
  ...
  if(nb_ener == 0) {
    ...
    L_acc = L_shl(L_acc, *sh_ener, &Overflow); /* >> if *sh_ener < 0 */
    ...
    sh1 = add(sh1, (16-marg[nb_ener]), &Overflow);
    L_x = 0L;
    for(i=0; i<nb_ener; i++) {
      temp = sub(sh1, sh_ener[i], &Overflow);
      L_acc = L_deposit_l(ener[i]);
      L_acc = L_shl(L_acc, temp, &Overflow);
      L_x = L_add(L_x, L_acc, &Overflow);
    }
    L_Extract(L_x, &hi, &lo);
...
)
{
  ...
  Log2(L_x, &exp, &frac);
  temp = sub(exp, sh, &Overflow);
  e_tmp = shl(temp, 10, &Overflow);
  e_tmp = add(e_tmp, mult_r(frac, 1024), &Overflow); /* 2^10 x log2(L_x . 2^-sh) */
  /* log2(ener) = 10log10(ener) / K */
  /* K = 10 Log2 / Log10 */

  temp = sub(e_tmp, -2721, &Overflow);      /* -2721 -> -8dB */
  if(temp <= 0) {
    *enerq = -12;
    return(0);
  }

  temp = sub(e_tmp, 22111, &Overflow);      /* 22111 -> 65 dB */  
  if(temp > 0) {
    *enerq = 66;
    return(31);
  }

  temp = sub(e_tmp, 4762, &Overflow);       /* 4762 -> 14 dB */
  if(temp <= 0){
    e_tmp = add(e_tmp, 3401, &Overflow);
    index = mult(e_tmp, 24, &Overflow);
    if (index < 1) index = 1;
    *enerq = sub(shl(index, 2, &Overflow), 8, &Overflow);
    return(index);
  }

  e_tmp = sub(e_tmp, 340, &Overflow);
  index = sub(shr(mult(e_tmp, 193, &Overflow), 2, &Overflow), 1, &Overflow);
  if (index < 6) index = 6;
  *enerq = add(shl(index, 1, &Overflow), 4, &Overflow);
  return(index);
}
