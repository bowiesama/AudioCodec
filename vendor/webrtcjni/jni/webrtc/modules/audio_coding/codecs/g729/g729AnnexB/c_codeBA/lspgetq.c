...
)
{
  ...
  for ( j = 0 ; j < NC ; j++ )
    buf[j] = add( lspcb1[code0][j], lspcb2[code1][j], &Overflow);

  for ( j = NC ; j < M ; j++ )
    buf[j] = add( lspcb1[code0][j], lspcb2[code2][j], &Overflow);
...
)
{
  ...
  for ( j = 1 ; j < NC ; j++ ) {
    diff = sub( buf[j-1], buf[j], &Overflow);
    tmp = shr( add( diff, gap, &Overflow), 1, &Overflow);

    if ( tmp >  0 ) {
      buf[j-1] = sub( buf[j-1], tmp, &Overflow);
      buf[j]   = add( buf[j], tmp, &Overflow);
    }
  }
  return;
}
...
)
{
  ...
  for ( j = 0 ; j < M ; j++ ) {
    L_acc = L_mult( lsp_ele[j], fg_sum[j], &Overflow);
    for ( k = 0 ; k < MA_NP ; k++ )
      L_acc = L_mac( L_acc, freq_prev[k][j], fg[k][j], &Overflow);
...
)
{
  ...
  for ( j = 0 ; j < M ; j++ ) {
    ...
    for ( k = 0 ; k < MA_NP ; k++ )
      L_temp = L_msu( L_temp, freq_prev[k][j], fg[k][j], &Overflow);

    temp = extract_h(L_temp);
    L_temp = L_mult( temp, fg_sum_inv[j], &Overflow);
    lsp_ele[j] = extract_h( L_shl( L_temp, 3, &Overflow) );
...
)
{
  ...
  for(j=0; j<M-1; j++) {
    ...
    L_diff = L_sub( L_acc, L_accb, &Overflow);
  ...
  if( sub(buf[0], L_LIMIT, &Overflow) <0 ) {
    buf[0] = L_LIMIT;
  ...
  for(j=0; j<M-1; j++) {
    ...
    if( L_sub(L_diff, GAP3, &Overflow)<0L ) {
      buf[j+1] = add( buf[j], GAP3, &Overflow);
    }
  }

  if( sub(buf[M-1],M_LIMIT, &Overflow)>0 ) {
    buf[M-1] = M_LIMIT;
