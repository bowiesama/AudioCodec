...
)
{
   ...
   *-------------------------------*/
   ...
   L_tmp = 0;
   for(i=0; i<L_subfr; i++)
     L_tmp = L_mac(L_tmp, code[i], code[i], &Overflow);
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = Mpy_32_16(exp, frac, -24660);   /* Q0 Q15 Q13 -> ^Q14     */
                                           ...
                                           /* lo:Q15+Q13-15+1        */
                                           /* -24660[Q13]=-3.0103    */
   L_tmp = L_mac(L_tmp, 32588, 32, &Overflow);        /* 32588*32[Q14]=127.298  */
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = L_shl(L_tmp, 10, &Overflow);             /* From Q14 to Q24 */
   for(i=0; i<4; i++)
     L_tmp = L_mac(L_tmp, pred[i], past_qua_en[i], &Overflow); /* Q13*Q10 ->Q24 */
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = L_mult(*gcode0, 5439, &Overflow);       /* *0.166 in Q15, result in Q24*/
   L_tmp = L_shr(L_tmp, 8, &Overflow);             /* From Q24 to Q16             */
   L_Extract(L_tmp, &exp, &frac);       /* Extract exponent of gcode0  */

   *gcode0 = extract_l(Pow2(14, frac)); /* Put 14 as exponent so that  */
                                        /* output of Pow2() will be:   */
                                        /* 16768 < Pow2() <= 32767     */
   *exp_gcode0 = sub(14,exp, &Overflow);
}
...
)
{
   ...
   *----------------------------------------------------------------------*/
   ...
   L_acc = L_Comp( sub(exp, 13, &Overflow), frac);         /* L_acc:Q16           */
   tmp = extract_h( L_shl( L_acc, 13, &Overflow) );       /* tmp:Q13           */
   past_qua_en[0] = mult( tmp, 24660, &Overflow);        /* past_qua_en[]:Q10 */
}
...
)
{
   ...
   L_tmp = 0;                                           /* Q10 */
   for(i=0; i<4; i++)
      L_tmp = L_add( L_tmp, L_deposit_l( past_qua_en[i] ), &Overflow );
   av_pred_en = extract_l( L_shr( L_tmp, 2, &Overflow));
   av_pred_en = sub( av_pred_en, 4096, &Overflow);      /* Q10 */

   if( sub(av_pred_en, -14336, &Overflow) < 0 ){
