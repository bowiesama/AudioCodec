...
)
{
   ...
   if(bfi != 0){
      *gain_pit = mult( *gain_pit, 29491, &Overflow);      /* *0.9 in Q15 */
      if (sub( *gain_pit, 29491, &Overflow) > 0) *gain_pit = 29491;
      *gain_cod = mult( *gain_cod, 32111 , &Overflow);      /* *0.98 in Q15 */
   ...
   index1 = imap1[ shr(index,NCODE2_B, &Overflow) ] ;
   index2 = imap2[ index & (NCODE2-1) ] ;
   *gain_pit = add( gbk1[index1][0], gbk2[index2][0] , &Overflow);
