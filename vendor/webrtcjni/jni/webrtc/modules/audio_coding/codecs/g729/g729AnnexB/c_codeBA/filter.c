...
)
{
   ...
   for (n = 0; n < L; n++)
   {
     s = 0;
     for (i = 0; i <= n; i++)
       s = L_mac(s, x[i], h[n-i], &Overflow);

     s    = L_shl(s, 3, &Overflow);                   /* h is in Q12 and saturation */
     y[n] = extract_h(s);
   }
...
)
{
  ...
  for (i = 0; i < lg; i++)
  {
    s = L_mult(x[i], a[0], &Overflow);
    for (j = 1; j <= M; j++)
      s = L_msu(s, a[j], yy[-j], &Overflow);

    s = L_shl(s, 3, &Overflow);
    *yy++ = W_round(s, &Overflow);
  }
...
)
{
  ...
  for (i = 0; i < lg; i++)
  {
    s = L_mult(x[i], a[0], &Overflow);
    for (j = 1; j <= M; j++)
      s = L_mac(s, a[j], x[i-j], &Overflow);

    s = L_shl(s, 3, &Overflow);
    y[i] = W_round(s, &Overflow);
  }
  return;
}
