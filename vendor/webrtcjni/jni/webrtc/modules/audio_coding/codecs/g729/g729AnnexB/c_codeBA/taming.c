...
{
  Word16 i;
  for(i=0; i<4; i++) coder->L_exc_err[i] = 0x00004000L;   /* Q14 */
}
...
Word16 test_err(  /* (o) flag set to 1 if taming is necessary  */
...
)
 {
    ...
    if(T0_frac > 0) {
        t1 = add(T0, 1, &Overflow);
    }
    else {
        t1 = T0;
    }

    i = sub(t1, (L_SUBFR+L_INTER10), &Overflow);
    if(i < 0) {
        i = 0;
    }
    zone1 = tab_zone[i];

    i = add(t1, (L_INTER10 - 2), &Overflow);
    zone2 = tab_zone[i];

    L_maxloc = -1L;
    flag = 0 ;
    for(i=zone2; i>=zone1; i--) {
        L_acc = L_sub(coder->L_exc_err[i], L_maxloc, &Overflow);
        if(L_acc > 0L) {
                L_maxloc = coder->L_exc_err[i];
        }
    }
    L_acc = L_sub(L_maxloc, L_THRESH_ERR, &Overflow);
    if(L_acc > 0L) {
        flag = 1;
    }
...
void update_exc_err(
...
)
 {
    ...
    L_worst = -1L;
    n = sub(T0, L_SUBFR, &Overflow);

    if(n < 0) {
        L_Extract(L_exc_err[0], &hi, &lo);
        L_temp = Mpy_32_16(hi, lo, gain_pit);
        L_temp = L_shl(L_temp, 1, &Overflow);
        L_temp = L_add(0x00004000L, L_temp, &Overflow);
        L_acc = L_sub(L_temp, L_worst, &Overflow);
        if(L_acc > 0L) {
                L_worst = L_temp;
        }
