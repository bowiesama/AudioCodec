...
)
{
  ...
  for(mode = 0; mode<MODE; mode++) {
    ...
    for( j = 0 ; j < NC ; j++ )
      buf[j] = add( lspcb1[cand_cur][j], lspcb2[index][j], &Overflow);
  ...
  code_ana[0] = shl( mode_index,NC0_B, &Overflow) | cand[mode_index];
  code_ana[1] = shl( tindex1[mode_index],NC1_B, &Overflow) | tindex2[mode_index];
...
)
{
  ...
  *cand = 0;
  ...
  for ( i = 0 ; i < NC0 ; i++ ) {
    L_tmp = 0;
    for ( j = 0 ; j < M ; j++ ) {
      tmp = sub(rbuf[j], lspcb1[i][j], &Overflow);
      L_tmp = L_mac( L_tmp, tmp, tmp, &Overflow);
    }

    L_temp = L_sub(L_tmp,L_dmin, &Overflow);
    if (  L_temp< 0L) {
      L_dmin = L_tmp;
      *cand = i;
    }
...
)
{
  ...
  for ( j = 0 ; j < NC ; j++ )
    buf[j] = sub(rbuf[j], lspcb1[j], &Overflow);
  ...
  *index = 0;
  ...
  for ( k1 = 0 ; k1 < NC1 ; k1++ ) {
    L_dist = 0;
    for ( j = 0 ; j < NC ; j++ ) {
      tmp = sub(buf[j], lspcb2[k1][j], &Overflow);
      tmp2 = mult( wegt[j], tmp , &Overflow);
      L_dist = L_mac( L_dist, tmp2, tmp, &Overflow);
    }

    L_temp =L_sub(L_dist,L_dmin, &Overflow);
    if ( L_temp <0L ) {
      L_dmin = L_dist;
      *index = k1;
    }
...
)
{
  ...
  *L_tdist = 0;
  for ( j = 0 ; j < M ; j++ ) {
    /* tmp = (buf - rbuf)*fg_sum */
    tmp = sub( buf[j], rbuf[j], &Overflow);
    tmp = mult( tmp, fg_sum[j], &Overflow);

    /* *L_tdist += wegt * tmp * tmp */
    L_acc = L_mult( wegt[j], tmp, &Overflow);
    tmp2 = extract_h( L_shl( L_acc, 4, &Overflow) );
    *L_tdist = L_mac( *L_tdist, tmp2, tmp, &Overflow);
  }
...
)
{
  ...
  *mode_index = 0;
  L_temp =L_sub(L_tdist[1] ,L_tdist[0], &Overflow);
  if (  L_temp<0L){
    *mode_index = 1;
  }
  return;
...
)
{
  ...
  buf[0] = sub( flsp[1], (PI04+8192), &Overflow);           /* 8192:1.0(Q13) */

  for ( i = 1 ; i < M-1 ; i++ ) {
    tmp = sub( flsp[i+1], flsp[i-1], &Overflow);
    buf[i] = sub( tmp, 8192, &Overflow);
  }

  buf[M-1] = sub( (PI92-8192), flsp[M-2], &Overflow);
  ...
  for ( i = 0 ; i < M ; i++ ) {
    if ( buf[i] > 0 ){
      ...
      L_acc = L_mult( buf[i], buf[i], &Overflow);           /* L_acc in Q27 */
      tmp = extract_h( L_shl( L_acc, 2, &Overflow) );       /* tmp in Q13 */

      L_acc = L_mult( tmp, CONST10, &Overflow);             /* L_acc in Q25 */
      tmp = extract_h( L_shl( L_acc, 2, &Overflow));       /* tmp in Q11 */

      wegt[i] = add( tmp, 2048, &Overflow);                 /* wegt in Q11 */
    }
  }

  /* */
  L_acc = L_mult( wegt[4], CONST12, &Overflow);             /* L_acc in Q26 */
  wegt[4] = extract_h( L_shl( L_acc, 1, &Overflow));       /* wegt in Q11 */

  L_acc = L_mult( wegt[5], CONST12, &Overflow);             /* L_acc in Q26 */
  wegt[5] = extract_h( L_shl( L_acc, 1, &Overflow));       /* wegt in Q11 */

  /* wegt: Q11 -> normalized */
  tmp = 0;
  for ( i = 0; i < M; i++ ) {
    if ( sub(wegt[i], tmp, &Overflow) > 0 ) {
      tmp = wegt[i];
    }
  }
  ...
  sft = norm_s(tmp);
  for ( i = 0; i < M; i++ ) {
    wegt[i] = shl(wegt[i], sft, &Overflow);                  /* wegt in Q(11+sft) */
  }
...
void Get_freq_prev(CodState *coder, Word16 x[MA_NP][M])
{
  ...
  for (i=0; i<MA_NP; i++)
    Copy(&coder->freq_prev[i][0], &x[i][0], M);
}
...
void Update_freq_prev(CodState *coder, Word16 x[MA_NP][M])
{
  ...
  for (i=0; i<MA_NP; i++)
    Copy(&x[i][0], &coder->freq_prev[i][0], M);
}
