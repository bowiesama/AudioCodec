...
)
{
       ...
       *------------------------------------------------------------------*/
      for(i=0; i<L_SUBFR; i++) {
         scaled_y2[i] = shr(y2[i], 3, &Overflow);        }

      /* Compute scalar product <y2[],y2[]> */
      L_acc = 1;                       /* Avoid case of all zeros */
      for(i=0; i<L_SUBFR; i++)
         L_acc = L_mac(L_acc, scaled_y2[i], scaled_y2[i], &Overflow);    /* L_acc:Q19 */

      exp      = norm_l(L_acc);
      y2y2     = W_round( L_shl(L_acc, exp, &Overflow) , &Overflow);
      exp_y2y2 = add(exp, 19-16, &Overflow);                          /* Q[19+exp-16] */
      ...
      L_acc = 1;                       /* Avoid case of all zeros */
      for(i=0; i<L_SUBFR; i++)
         L_acc = L_mac(L_acc, xn[i], scaled_y2[i], &Overflow);           /* L_acc:Q10 */

      exp      = norm_l(L_acc);
      xny2     = W_round( L_shl(L_acc, exp, &Overflow), &Overflow );
      exp_xny2 = add(exp, 10-16, &Overflow);                          /* Q[10+exp-16] */

      g_coeff[3]     = negate(xny2);
      exp_g_coeff[3] = sub(exp_xny2,1, &Overflow);                   /* -2<xn,y2> */

      /* Compute scalar product <y1[],y2[]> */
      L_acc = 1;                       /* Avoid case of all zeros */
      for(i=0; i<L_SUBFR; i++)
         L_acc = L_mac(L_acc, y1[i], scaled_y2[i], &Overflow);           /* L_acc:Q10 */

      exp      = norm_l(L_acc);
      y1y2     = W_round( L_shl(L_acc, exp, &Overflow) , &Overflow);
      exp_y1y2 = add(exp, 10-16, &Overflow);                          /* Q[10+exp-16] */

      g_coeff[4]     = y1y2;
      exp_g_coeff[4] = sub(exp_y1y2,1, &Overflow);    ;                /* 2<y1,y2> */
...
)
{
   ...
   for (i = 0; i < L_SUBFR; i++)
   {
     s = 0;
     for (j = i; j <  L_SUBFR; j++)
       s = L_mac(s, X[j], h[j-i], &Overflow);

     y32[i] = s;

     s = L_abs(s);
     L_temp =L_sub(s,max, &Overflow);
     if(L_temp>0L) {
        max = s;
     }
   }
   ...
   for(i=0; i<L_SUBFR; i++) {
     D[i] = extract_l( L_shr(y32[i], j, &Overflow) );
   }
