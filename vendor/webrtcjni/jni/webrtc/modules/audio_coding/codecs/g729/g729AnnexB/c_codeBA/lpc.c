...
)
{
  ...
  *exp_R0 = 1;
  ...
  do {
    ...
    for(i=0; i<L_WINDOW; i++)
      sum = L_mac(sum, y[i], y[i], &Overflow);
    ...
    if(Overflow != 0)
    {
      for(i=0; i<L_WINDOW; i++)
      {
        y[i] = shr(y[i], 2, &Overflow);
      }
      *exp_R0 = add((*exp_R0), 4, &Overflow);
      Overflow = 1;
    }
  }while (Overflow != 0);
  ...
  norm = norm_l(sum);
  sum  = L_shl(sum, norm, &Overflow);
  L_Extract(sum, &r_h[0], &r_l[0]);     /* Put in DPF format (see oper_32b) */
  *exp_R0 = sub(*exp_R0, norm, &Overflow);
  ...
  for (i = 1; i <= m; i++)
  {
    ...
    for(j=0; j<L_WINDOW-i; j++)
      sum = L_mac(sum, y[j], y[j+i], &Overflow);
...
/* K = A[1] = -R[1] / R[0] */
  ...
  rc[0] = Kh;
  t0 = L_shr(t0,4, &Overflow);                     /* A[1] in Q27      */
...
/*  Alpha = R[0] * (1-K**2) */
  ...
  t0 = L_sub( (Word32)0x7fffffffL, t0 , &Overflow); /* 1 - K*K  in Q31 */
 ...
 *--------------------------------------*/
  ...
  for(i= 2; i<=M; i++)
  {
    ...
    t0 = 0;
    for(j=1; j<i; j++)
      t0 = L_add(t0, Mpy_32(Rh[j], Rl[j], Ah[i-j], Al[i-j]), &Overflow);

    t0 = L_shl(t0,4, &Overflow);                  /* result in Q27 -> convert to Q31 */
                                       /* No overflow possible            */
    t1 = L_Comp(Rh[i],Rl[i]);
    t0 = L_add(t0, t1, &Overflow);                /* add R[i] in Q31                 */
    ...
    t2 = L_shl(t2, alp_exp, &Overflow);           /* denormalize; compare to Alpha   */
    L_Extract(t2, &Kh, &Kl);           /* K in DPF                        */
    rc[i-1] = Kh;

    /* Test for unstable filter. If unstable keep old A(z) */

    if (sub(abs_s(Kh), 32750, &Overflow) > 0)
    {
      for(j=0; j<=M; j++)
      {
        A[j] = coder->old_A[j];
    ...
    for(j=1; j<i; j++)
    {
      ...
      t0 = L_add(t0, L_Comp(Ah[j], Al[j]), &Overflow);
      L_Extract(t0, &Anh[j], &Anl[j]);
    }
    t2 = L_shr(t2, 4, &Overflow);       /* t2 = K in Q31 ->convert to Q27  */
    ...
    j = norm_l(t0);
    t0 = L_shl(t0, j, &Overflow);
    L_Extract(t0, &alp_h, &alp_l);         /* DPF format    */
    alp_exp = add(alp_exp, j, &Overflow);             /* Add normalization to alp_exp */
  ...
  *Err = shr(alp_h, alp_exp, &Overflow);
  ...
  {
    t0   = L_Comp(Ah[i], Al[i]);
    coder->old_A[i] = A[i] = W_round(L_shl(t0, 1, &Overflow), &Overflow);
  }
  coder->old_rc[0] = rc[0];
  coder->old_rc[1] = rc[1];
 ...
 *-------------------------------------------------------------*/
 ...
 {
   Overflow = 0;
   t0 = L_mult(a[i+1], 16384, &Overflow);          /* x = (a[i+1] + a[M-i]) >> 1        */
   t0 = L_mac(t0, a[M-i], 16384, &Overflow);       /*    -> From Q12 to Q11             */
   x  = extract_h(t0);
   if ( Overflow ) {
     ovf_coef = 1;      }

   Overflow = 0;
   f1[i+1] = sub(x, f1[i], &Overflow);    /* f1[i+1] = a[i+1] + a[M-i] - f1[i] */
   if ( Overflow ) {
     ovf_coef = 1;      }

   Overflow = 0;
   t0 = L_mult(a[i+1], 16384, &Overflow);          /* x = (a[i+1] - a[M-i]) >> 1        */
   t0 = L_msu(t0, a[M-i], 16384, &Overflow);       /*    -> From Q12 to Q11             */
   x  = extract_h(t0);
   if ( Overflow ) {
     ovf_coef = 1;      }

   Overflow = 0;
   f2[i+1] = add(x, f2[i], &Overflow);    /* f2[i+1] = a[i+1] - a[M-i] + f2[i] */
   if ( Overflow ) {
     ovf_coef = 1;      }
 }
 ...
 if ( ovf_coef ) {
   ...
   for (i = 0; i< NC; i++)
   {
     t0 = L_mult(a[i+1], 8192, &Overflow);          /* x = (a[i+1] + a[M-i]) >> 1        */
     t0 = L_mac(t0, a[M-i], 8192, &Overflow);       /*    -> From Q11 to Q10             */
     x  = extract_h(t0);
     f1[i+1] = sub(x, f1[i], &Overflow);            /* f1[i+1] = a[i+1] + a[M-i] - f1[i] */

     t0 = L_mult(a[i+1], 8192, &Overflow);          /* x = (a[i+1] - a[M-i]) >> 1        */
     t0 = L_msu(t0, a[M-i], 8192, &Overflow);       /*    -> From Q11 to Q10             */
     x  = extract_h(t0);
     f2[i+1] = add(x, f2[i], &Overflow);            /* f2[i+1] = a[i+1] - a[M-i] + f2[i] */
   }
 }
 ...
 *-------------------------------------------------------------*/
 ...
 while ( (nf < M) && (j < GRID_POINTS) )
 {
   ...
   L_temp = L_mult(ylow ,yhigh, &Overflow);
   if ( L_temp <= (Word32)0)
   {
     ...
     for (i = 0; i < 2; i++)
     {
       xmid = add( shr(xlow, 1, &Overflow) , shr(xhigh, 1, &Overflow), &Overflow); /* xmid = (xlow + xhigh)/2 */

       ymid = (*pChebps)(xmid,coef,NC);

       L_temp = L_mult(ylow,ymid, &Overflow);
       if ( L_temp <= (Word32)0)
       {
         yhigh = ymid;
         xhigh = xmid;
     ...
     *-------------------------------------------------------------*/
     ...
     x   = sub(xhigh, xlow, &Overflow);
     y   = sub(yhigh, ylow, &Overflow);
     ...
     else
     {
       ...
       y   = abs_s(y);
       exp = norm_s(y);
       y   = shl(y, exp, &Overflow);
       y   = div_s( (Word16)16383, y);
       t0  = L_mult(x, y, &Overflow);
       t0  = L_shr(t0, sub(20, exp, &Overflow), &Overflow);
       y   = extract_l(t0);            /* y= (xhigh-xlow)/(yhigh-ylow) in Q11 */

       if(sign < 0) y = negate(y);

       t0   = L_mult(ylow, y, &Overflow);                  /* result in Q26 */
       t0   = L_shr(t0, 11, &Overflow);                    /* result in Q15 */
       xint = sub(xlow, extract_l(t0), &Overflow);         /* xint = xlow - ylow*y */
     }

     lsp[nf] = xint;
     xlow    = xint;
     nf =add(nf,1, &Overflow);
...
static Word16 Chebps_11(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q24. */
  ...
  t0 = L_mult(x, 512, &Overflow);                  /* 2*x in Q24          */
  t0 = L_mac(t0, f[1], 4096, &Overflow);           /* + f[1] in Q24       */
  L_Extract(t0, &b1_h, &b1_l);          /* b1 = 2*x + f[1]     */

  for (i = 2; i<n; i++)
  {
    t0 = Mpy_32_16(b1_h, b1_l, x);      /* t0 = 2.0*x*b1              */
    t0 = L_shl(t0, 1, &Overflow);
    t0 = L_mac(t0,b2_h,(Word16)-32768L, &Overflow);/* t0 = 2.0*x*b1 - b2         */
    t0 = L_msu(t0, b2_l, 1, &Overflow);
    t0 = L_mac(t0, f[i], 4096, &Overflow);         /* t0 = 2.0*x*b1 - b2 + f[i]; */
  ...
  t0 = L_mac(t0, b2_h,(Word16)-32768L, &Overflow); /* t0 = x*b1 - b2          */
  t0 = L_msu(t0, b2_l, 1, &Overflow);
  t0 = L_mac(t0, f[i], 2048, &Overflow);           /* t0 = x*b1 - b2 + f[i]/2 */

  t0 = L_shl(t0, 6, &Overflow);                    /* Q24 to Q30 with saturation */
...
static Word16 Chebps_10(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q23. */
  ...
  t0 = L_mult(x, 256, &Overflow);                  /* 2*x in Q23          */
  t0 = L_mac(t0, f[1], 4096, &Overflow);           /* + f[1] in Q23       */
  L_Extract(t0, &b1_h, &b1_l);          /* b1 = 2*x + f[1]     */
  ...
  t0 = L_shl(t0, 7, &Overflow);                    /* Q23 to Q30 with saturation */
  cheb = extract_h(t0);                 /* Result in Q14              */
