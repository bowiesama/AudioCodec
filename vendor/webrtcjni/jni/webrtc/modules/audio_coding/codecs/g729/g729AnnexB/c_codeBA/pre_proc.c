...
{
  Word16 *signal = coder->new_speech; /* input/output signal */
  ...
  for(i=0; i<lg; i++)
  {
     ...
     L_tmp     = L_add(L_tmp, Mpy_32_16(coder->y2_hi, coder->y2_lo, a140[2]), &Overflow);
     L_tmp     = L_mac(L_tmp, coder->x0, b140[0], &Overflow);
     L_tmp     = L_mac(L_tmp, coder->x1, b140[1], &Overflow);
     L_tmp     = L_mac(L_tmp, x2, b140[2], &Overflow);
     L_tmp     = L_shl(L_tmp, 3, &Overflow);      /* Q28 --> Q31 (Q12 --> Q15) */
     signal[i] = W_round(L_tmp, &Overflow);
