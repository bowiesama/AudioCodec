...
/* Runtime statistics */
#include <time.h>
#define CLOCKS_PER_SEC_G729  100000
...
int main(int argc, char* argv[])
{
    ...
    G729EncInst *G729enc_inst;
    G729DecInst *G729dec_inst;
    int err;
    ...
    if (argc!=5)  {
        ...
        printf("\n");
        printf("Usage:\n\n");
        printf("./testG729.exe framelength infile outbitfile outspeechfile \n\n");
        printf("with:\n");
    ...
    if ((inp = fopen(inname,"rb")) == NULL) {
        printf("  G.729: Cannot read file %s.\n", inname);
        exit(1);
    }
    if ((outbitp = fopen(outbit,"wb")) == NULL) {
        printf("  G.729: Cannot write file %s.\n", outbit);
        exit(1);
    }
    ...
    /* Create and init */
    WebRtcG729_CreateEnc((G729EncInst **)&G729enc_inst);
    WebRtcG729_CreateDec((G729DecInst **)&G729dec_inst);
    WebRtcG729_EncoderInit((G729EncInst *)G729enc_inst, 0);
    WebRtcG729_DecoderInit((G729DecInst *)G729dec_inst);
    ...
    while (endfile == 0) {
        ...
        starttime = clock()/(double)CLOCKS_PER_SEC_G729;

        /* G.729 encoding + decoding */
        stream_len = WebRtcG729_Encode((G729EncInst *)G729enc_inst, shortdata, framelength, (uint8_t*)streamdata);
        err = WebRtcG729_Decode((G729DecInst *)G729dec_inst, streamdata, stream_len, decoded, speechType);

        /* Stop clock after call to encoder and decoder */
        runtime += (double)((clock()/(double)CLOCKS_PER_SEC_G729)-starttime);
    ...
    WebRtcG729_FreeEnc((G729EncInst *)G729enc_inst);
    WebRtcG729_FreeDec((G729DecInst *)G729dec_inst);

    length_file = ((double)framecnt*(double)framelength/16000);
    printf("\n\nLength of speech file: %.1f s\n", length_file);
    printf("Time to run G.729:      %.2f s (%.2f %% of realtime)\n\n", runtime, (100*runtime/length_file));
    printf("---------------------END----------------------\n");
